import pandas as pd
# from sklearn.feature_selection import mutual_info_regression ### I HAVE PROBLEM WITH scipy
...
def ts_corr(x, y, window):
    # res = Parallel(n_jobs=cpu_count())(delayed(rolling_apply)(dcor, window, x[col], y[col]) for col in x) # I have changed this
    # res = Parallel(n_jobs=2)(delayed(rolling_apply)(dcor, window, x[col], y[col]) for col in x)
    res = [rolling_apply(dcor, window, x[col], y[col]) for col in x]
...
class Alphas101:
    ...
    def __init__(self):
        print("test")
        # memory = Memory("D:/temp/alpha101", verbose=0)
...
def alpha01_main(data):
  alphas101 = Alphas101()
  features = alphas101.transform(data)
  return features
